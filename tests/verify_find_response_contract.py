...
def main() -> int:
    ...
    if response.get("matched") is not True:
        failures.append("find response did not match")
    if response.get("matched_address") != "TA11111111111111111111111111CDEFG":
        failures.append("matched_address mismatch")
    ...
    if not isinstance(encrypted, str) or not encrypted.startswith("-----BEGIN AGE ENCRYPTED FILE-----"):
        failures.append("missing age ciphertext")
    failures.extend(collect_forbidden_markers(response))
        ...
        json.dumps(
            {
                ...
                "notes": [
                    "Uses fake local GPU and fake local age binary only.",
